...
export default function AIInsightsPanel({ objA, objB }: Props) {
  ...
  const [aiSource, setAiSource]   = useState<"granite" | "offline" | null>(null);
  ...
  const generate = async () => {
    ...
    try {
      ...
      setInsight(answer);
      setStreaming(true);
      setAiSource(data.source === "granite" ? "granite" : "offline");
    } catch {
          ...
          <div>
            ...
            <p className="text-purple-300/70 text-xs">
              {aiSource === "granite"
                ...
                : aiSource === "offline"
                ? "📡 Offline RAG Engine"
                : "Powered by local IBM Granite AI"}
            </p>
          </div>
        </div>
              ...
              <div className="mt-5 flex items-center justify-between flex-wrap gap-2">
                ...
                {aiSource && (
                  <span
                    ...
                    style={{
                      background: aiSource === "granite" ? "rgba(14,165,233,0.15)" : "rgba(147,197,253,0.08)",
                      border: aiSource === "granite" ? "1px solid rgba(14,165,233,0.3)" : "1px solid rgba(147,197,253,0.15)",
                      color: aiSource === "granite" ? "#38bdf8" : "rgba(147,197,253,0.6)",
                    }}
                  >
                    {aiSource === "granite" ? "🔷 IBM Granite 3.3" : "📡 Offline mode"}
                  </span>
                )}
              </div>
            </motion.div>
