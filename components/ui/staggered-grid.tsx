...
export interface GridTile {
    /** Which icon to show: 'github' | 'bob' | 'portfolio' */
    type: 'github' | 'bob' | 'portfolio'
    label: string
    href?: string
    /** Optional src for image-based icons (bob / portfolio) */
    iconSrc?: string
    /** Tooltip / sub-label shown on hover */
    hoverLabel?: string
}
...
export interface StaggeredGridProps {
    ...
    /** Override the three repeating tile icons. Default: github / bob / portfolio */
    tiles?: [GridTile, GridTile, GridTile]
    className?: string
    showFooter?: boolean
                        ...
                        if (typeof item === 'string') {
                            ...
                            const Wrapper = tile.href
                                ? ({ children }: { children: React.ReactNode }) => (
                                    <a href={tile.href} target="_blank" rel="noopener noreferrer" className="contents">
                                        {children}
                                    </a>
                                )
                                    ...
                                    <Wrapper>
                                        <div className="grid__item-img w-full h-full [backface-visibility:hidden] will-change-transform rounded-xl overflow-hidden shadow-sm border border-zinc-200 dark:border-zinc-900 bg-zinc-100 dark:bg-zinc-950 flex items-center justify-center transition-all duration-500 ease-out group-hover:scale-105 group-hover:shadow-xl group-hover:border-transparent">
                                            ...
                                            <div className="relative z-10 flex flex-col items-center justify-center gap-3">
                                                {/* Icon */}
                                                {tile.type === 'github' ? (
                                                    <FaGithub className="w-8 h-8 text-zinc-400 dark:text-zinc-500 transition-all duration-300 group-hover:text-white group-hover:scale-110" />
                                                ) : tile.iconSrc ? (
                                                    // eslint-disable-next-line @next/next/no-img-element
                                                    <img src={tile.iconSrc} alt={tile.label} className="w-10 h-10 object-contain transition-all duration-300 group-hover:scale-110 drop-shadow-[0_0_8px_rgba(147,197,253,0.6)]" />
                                                ) : null}
                                                ...
                                                <div className="text-center opacity-0 group-hover:opacity-100 transform translate-y-2 group-hover:translate-y-0 transition-all duration-300 delay-75">
                                                    <span className="block text-[10px] font-medium text-white/90 uppercase tracking-wider mb-0.5">{tile.hoverLabel ?? tile.label}</span>
                                                    <span className="block text-sm font-bold text-white tracking-tight">{tile.label}</span>
                                                </div>
                                            </div>
                                        </div>
                                    </Wrapper>
